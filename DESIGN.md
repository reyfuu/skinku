...
## Colors
- **Primary** (#982121): Terracotta — CTAs, active states, links, focus rings, progress indicators
- **Primary Hover** (#7A1A1A): Deep crimson — hover states on primary elements
...
## Elevation
Cards rest flat with a 1px warm border (#D6D3D1) and gain a soft shadow on hover (0 4px 16px rgba(28,25,23,0.06)). Active/selected cards get a 2px left border in deep red. Primary buttons gain a warm glow on hover (0 4px 12px rgba(152,33,33,0.25)). Modals use a larger shadow (0 24px 48px rgba(28,25,23,0.12)) with a backdrop blur. The nav is transparent with backdrop-blur, gaining a 1px bottom border on scroll.

## Components
- **Buttons**: Primary uses deep red (#982121) fill with white text, 8px radius, semibold weight. Secondary uses transparent bg with 1px stone border (#D6D3D1), warm text. Ghost has no border, just text color. Destructive uses red bg with white text. All buttons have 150ms transition. Sizes: small (32px height, 12px padding), medium (40px, 16px), large (48px, 24px).
...
- **Inputs**: 1px border (#D6D3D1), surface background (#F5F5F4), 8px radius, 12px padding, 16px font. Focus: border turns deep red (#982121) with a warm ring (0 0 0 3px rgba(152,33,33,0.12)). Error: border turns red. Labels are 14px semibold above the input.
...
- **Navigation**: Sidebar layout, 256px width. Sidebar background is deep red (#982121) with white/light text. Logo and workspace name at top. Collapsible sections with chevron toggles. Active item: white left accent bar (3px) with slightly darker red tinted background (#7A1A1A).
...
## Do's and Don'ts
- Do use deep red (#982121) only for interactive elements, sidebar, and active states — never as decoration
...
- Don't place multiple deep red primary buttons in the same section — one primary CTA per view
